...
def custome_preprocess(file_path):

    random_roatation = round(random.uniform(1, 4))
    scale_percent = 60 # percent of original size

    if random_roatation == 1:
        ...
        img_ = np.arr
        label = '1'
    elif random_roatation == 2:
        img = cv2.imread(file_path)
        width = 96
        height = 96
        dim = (width, height)
        img_ = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
        img_ = cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE)
        label = '2'
    elif random_roatation == 3:
        img = cv2.imread(file_path)
        width = 96
        height = 96
        dim = (width, height)
        img_ = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
        img_ = cv2.rotate(img, cv2.ROTATE_90_CLOCKWISE)
        label = '3'
    elif random_roatation == 4:
        img = cv2.imread(file_path)
        width = 96
        height = 96
        dim = (width, height)
        img_ = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
        img_ = cv2.rotate(img, cv2.img_rotate_90_counterclockwise)
        label = '4'
...
class DataGenerator(keras.utils.Sequence):
	...
	def __getitem__(self, index):
		...
		images,labels = np.array([custome_preprocess(img) for img in images])
...
def loadData(db, val_split=0.2, sub_sample_size=-1):
    ...
    if db == "train":
        image_paths = glob.glob("/home/giuser/API/Image_Generator_Keras/images/*.jpg")
        labels = []
        if val_split == 0:
            ...
            train_data = DataGenerator(image_paths, labels, batch_size=BATCH_SIZE, augment=True, shuffle=True)
